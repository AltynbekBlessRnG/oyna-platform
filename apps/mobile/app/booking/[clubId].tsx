...
/** Конец сеанса словами, с переходом через полночь: 23:40 + 3 ч → 02:40. */
function formatEnd(time: string, durationHours: number): string {
  const [hours, minutes] = time.split(":").map(Number);
  return `${String((hours + durationHours) % 24).padStart(2, "0")}:${String(minutes).padStart(2, "0")}`;
}
...
export default function BookingScreen() {
  ...
  // Минуты выбираются свободно, поэтому «сегодня 12:35» может быть уже позади.
  const inThePast = new Date(startAt).getTime() < Date.now();
  ...
  const canContinue = step === 0 ? !inThePast : seatIds.length > 0;
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 14 }]}>
        ...
        {step === 0 && <View>
          ...
          <View style={styles.clock}>
            ...
            <Text style={styles.clockUntil}>до {formatEnd(time, duration)}</Text>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.picker}>
            {HOURS.map((item) => <Pressable key={item} accessibilityLabel={`${item} часов`} onPress={() => setHour(item)} style={[styles.unit, hour === item && styles.optionActive]}><Text style={[styles.unitText, hour === item && styles.optionTextActive]}>{item}</Text></Pressable>)}
          </ScrollView>
          <Text style={styles.pickerLabel}>Минуты</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.picker}>
            {MINUTES.map((item) => <Pressable key={item} accessibilityLabel={`${item} минут`} onPress={() => setMinute(item)} style={[styles.unit, minute === item && styles.optionActive]}><Text style={[styles.unitText, minute === item && styles.optionTextActive]}>{item}</Text></Pressable>)}
          </ScrollView>
          {inThePast ? <Text style={styles.pastWarning}>Это время уже прошло — выбери другое или следующий день.</Text> : null}
        ...
        {step === 1 && <View>
          ...
          <Text style={styles.subtitle}>{days[dayIndex].toLocaleDateString("ru-RU", { day: "numeric", month: "long" })} · {time}–{formatEnd(time, duration)}. Тариф и цена — у самого компьютера.</Text>
...
const styles = StyleSheet.create({
  ...
  title: { color: colors.text, fontSize: 28, fontWeight: "900", letterSpacing: -0.7 }, subtitle: { marginTop: 7, color: colors.muted, fontSize: 14, lineHeight: 20 }, days: { gap: 9, paddingVertical: 22 }, day: { width: 76, height: 82, borderRadius: 17, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center", gap: 6 }, optionActive: { backgroundColor: colors.primary, borderColor: colors.primary }, dayWeek: { color: colors.muted, fontSize: 11, textTransform: "capitalize" }, dayNumber: { color: colors.text, fontSize: 22, fontWeight: "800" }, optionTextActive: { color: colors.primaryText }, sectionTitle: { marginTop: 10, marginBottom: 11, color: colors.text, fontSize: 15, fontWeight: "800" }, clock: { marginBottom: 18, paddingVertical: 15, borderRadius: 17, backgroundColor: colors.surface, alignItems: "center" }, clockValue: { color: colors.text, fontSize: 38, fontWeight: "900", letterSpacing: 1 }, clockUntil: { marginTop: 3, color: colors.muted, fontSize: 12 }, pickerLabel: { marginBottom: 8, color: colors.muted, fontSize: 11, fontWeight: "700" }, picker: { gap: 8, paddingBottom: 16 }, unit: { width: 54, paddingVertical: 13, alignItems: "center", borderRadius: 13, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, unitText: { color: colors.text, fontSize: 14, fontWeight: "700" }, pastWarning: { marginBottom: 8, color: colors.danger, fontSize: 12, lineHeight: 18 }, duration: { flexDirection: "row", alignItems: "center", gap: 12 }, counter: { width: 46, height: 46, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center" }, durationValue: { height: 46, flex: 1, borderRadius: 14, backgroundColor: colors.surface, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, durationText: { color: colors.text, fontWeight: "800" },
